...
public class InGame : MonoBehaviour {
	...
	void pitcherNameSet(){
		Debug.Log (int.Parse(step_by_pitch.pitcher) + " vs " + current_pitcher_id);
			...
			if (!is_top) {
				for (int i = 0; i < CurrentState.list_h.Length; i++) {
					if (int.Parse(step_by_pitch.batter) == (int)CurrentState.list_h [i] [3]) {
						...
						GameObject.Find ("InGameUI").GetComponent<StepByPitch>().clearCount();
					}
				}
			} else {
				for (int i = 0; i < CurrentState.list_a.Length; i++) {
	...
	void halfSet(){
		Debug.Log (step_by_pitch.inning);
		if (!is_top && step_by_pitch.inning % 2 == 0) {
			GameObject.Find ("playerlist").GetComponent<LineUpEntry> ().setTopLineUp ();
			is_top = true;
		} else if(is_top && step_by_pitch.inning % 2 != 0) {
	...
	public void createRunner(){
		...
		Debug.Log ("step step : " + step_by_pitch.list_base.Count);
				...
				for (int j = 0; j < step_by_pitch.list_base.Count; j++) {
					if (CurrentState.runner [i].GetComponent<Fielder> ().player_code.Equals (lb [j] [0])) {
						...
						if (lb [j] [1].Equals ("1B")) {
							...
							Debug.Log (CurrentState.runner [i].name);
							//runner일 경우에만 
