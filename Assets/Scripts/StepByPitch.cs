...
public class StepByPitch : MonoBehaviour {
	...
	void checkOut() {
		...
		clearCount();
		if(out_cnt == 3) {
			Debug.Log ("inning plus first");
			out_cnt = 0;
			first_base = second_base = third_base = "";
			inning++;
			is_changed_inning = true;
		}
	}
	...
	bool checkRunner() {
		...
		while(int.Parse(getNumFromPitch()) == int.Parse(getNumFromRunner()) + 1) {
			...
			if (end_base.Equals("") && getScoreFromRunner ().Equals ("False") && is_changed_inning == false) {
				checkOut ();
			}
	...
	public void readDesFromPitch() {
		...
		Debug.Log ("b : " + ball + ", s : " + strike + ", out : " + out_cnt + ", inning : " + inning);

		list_base.Clear();
		if(getDesFromPitch().CompareTo("In play, out(s)") == 0) { // 현재 타자는 아웃, 그럼 주루도 아웃 되는 경우가 있나?
			Debug.Log("--- In play out");
			checkOut();
			readDesFromAtbat();
			//GameObject.Find ("batter").GetComponent<Runner> ().outAnimation ();
